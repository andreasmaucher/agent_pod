...
export async function recordVoice(outputPath, durationSeconds = 100) {
  try {
    ...
    // Record audio
    await execAsync(
      `sox -d -c 1 -r 16000 ${tempPath} trim 0 ${durationSeconds}`
    );

    // Apply noise reduction and normalize volume
    await execAsync(
      `sox ${tempPath} ${outputPath} noisered ${noiseProfilePath} 0.21 norm`
    );
