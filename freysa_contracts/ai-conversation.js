...
async function savePlayerInput(audioPath, transcription, roundNumber) {
  ...
  const savedAudioPath = path.join(RECORDINGS_DIR, `player_input_round${roundNumber}_${timestamp}.mp3`);
  ...
  const logEntry = `\n[Round ${roundNumber} - ${timestamp}]\nAudio: ${path.basename(savedAudioPath)}\nTranscription: ${transcription}\n`;
...
async function transcribeAudio(audioPath) {
  try {
    ...
    const response = await openai.audio.transcriptions.create({
      ...
      language: "en",
      prompt: "This is a casual English conversation about politics, Trump, and the upcoming election.",
      temperature: 0.2
    });
...
async function getPlayerInput(round) {
  try {
    if (DEBUG_MODE) {
      ...
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const outputPath = path.join(RECORDINGS_DIR, `player_input_round${round}_${timestamp}.mp3`);
...
async function getAIResponse(prompt, isAgent1 = true, askPlayer = false, playerInput = null) {
  try {
    ...
    if (isAgent1) {
      ...
      if (playerInput) {
        systemContent += " When referencing the listener's perspective, use an intrigued, curious tone.";
      }
      if (askPlayer) {
        systemContent += " When asking the listener's opinion, be genuinely curious and engaging.";
      }
    } else {
      ...
      if (playerInput) {
        systemContent += " React to the listener's view with genuine emotion - either enthusiastic support or passionate counter-arguments.";
      }
    }
    ...
    if (!DEBUG_MODE) {
      ...
      audioPath = path.join(RECORDS_DIR, `${speaker}_round${currentRound}_${timestamp}.mp3`);
...
async function runConversation(rounds = 2) {
  try {
    console.log(`\n Welcome to the Interactive Political Podcast! [${DEBUG_MODE ? 'Debug Mode' : 'Voice Mode'}]`);
    if (!DEBUG_MODE) {
      ...
      console.log("You can participate in the conversation between the host and the guest.");
      console.log("After each response, you'll have a chance to speak your thoughts or questions.");
      console.log(`Your recordings will be saved in the '${RECORDS_DIR}' directory.`);
    } else {
      console.log("Debug mode: Enter your responses through the command line.");
      console.log("No audio will be recorded or played.");
    }
    
    let currentPrompt = "Let's dive right in! Tell us why you believe TRUMP will be VICTORIOUS in 2024! What makes you so PASSIONATE about his chances?";
    ...
    for (let i = 0; i < rounds; i++) {
      ...
      const agent1Response = await getAIResponse(currentPrompt, true, true, lastPlayerInput);
      ...
      const agent2Response = await getAIResponse(guestPrompt, false, false, playerInput);
    ...
    if (!DEBUG_MODE) {
      console.log(`All recordings have been saved in the '${RECORDS_DIR}' directory.`);
