...
async function savePlayerInput(audioPath, transcription, roundNumber) {
  ...
  const savedAudioPath = path.join(recordingDir, `player_input_round${roundNumber}_${timestamp}.mp3`);
...
async function getAIResponse(prompt, isAgent1 = true) {
  ...
  const systemPrompt = isAgent1 
    ? "You are a podcast host. Keep your questions very short and direct, maximum 2 sentences. Focus on the most controversial aspects to get engaging responses."
    : "You are a passionate Trump supporter. Give short, energetic responses focusing on one key point at a time. Maximum 3 sentences. Be bold but respectful.";
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      { role: "system", content: "Keep your response under 10 seconds when spoken. Be very concise." }
    ],
...
async function runConversation(rounds = 2) {
  try {
    ...
    for (let i = 0; i < rounds; i++) {
      ...
      await textToSpeech(agent1Response, `./interviewer_round${i + 1}.mp3`, "onyx");
      
      // Guest responds
      const agent2Response = await getAIResponse(agent1Response, false);
      await textToSpeech(agent2Response, `./guest_round${i + 1}.mp3`, "shimmer");
    ...
    console.log(`Your recordings have been saved in the '${RECORDINGS_DIR}' directory.`);
  } catch (error) {
